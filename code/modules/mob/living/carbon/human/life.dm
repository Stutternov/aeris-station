...
// If a limb was missing, regrow
	if(LAZYLEN(organs) < 7)
		...
		for(var/tag in tags_to_grow)
			...
			var/datum/body_modification/BM = user_pref ? user_pref.get_modification(tag) : (upper_body_nature == MODIFICATION_ORGANIC) ? new /datum/body_modification/none : new /datum/body_modification/limb/prosthesis/serbian
